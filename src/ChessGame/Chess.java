...
public class Chess extends JPanel{
	...
	public void paintComponent(Graphics g) {
		...
		if (selectedRow != null && rawX < (400 + initialX) && rawY > (25 + initialY)
			...
			if ((board.whiteToMove && board.getPiece(selectedRow, selectedCol).getClass().getName().charAt(7+10) == 'w') ||
				(!board.whiteToMove && board.getPiece(selectedRow, selectedCol).getClass().getName().charAt(7+10) == 'b')) {
				tileSelected = true;
		...
		for (int r = 0; r < 8; r++) {
			for (int c = 0; c < 8; c++) {
				...
				if (reverseDrawing) {
					pieceName = board.getPiece(7 - r, 7 - c).getClass().getName().substring(7+10);
				}
				else pieceName = board.getPiece(r, c).getClass().getName().substring(7+10);
				...
				if (board.isMoveValid(selectedRow, selectedCol, newSelectedRow, newSelectedCol)) {
					...
					if (board.whiteInCheck || board.blackInCheck) {
						if (board.isCheckmate()) {
							...
							System.out.println(board.showMoves.getText());
					...
					for (int r = 0; r < 8; r++) {
						for (int c = 0; c < 8; c++) {
							if (board.whiteToMove && board.getPiece(r, c) != null) {
								if (board.getPiece(r, c).getClass().getName().substring(7+10).equals("blackPawn")) {
								...
								if (board.getPiece(r, c) != null && board.getPiece(r, c).getClass().getName().substring(7+10).equals("whitePawn")) {
									if (board.getPiece(r, c).possibleEnPassant()) {
										board.getPiece(r, c).switchEnPassant();
									}
								}
	...
	class AllEncompassingListener extends WindowAdapter implements ActionListener,Runnable {
		...
		private ChessPiece[][] fileParser(File file) {
			...
			//set movecount
			board.setMoveCount(Integer.parseInt(firstLine.substring(1+10)));
			while (input.hasNextLine()) {
				...
				if (nextLine.substring(7+10, 8+10).equals("m")) piece.moved();
