...
public class whiteKing extends ChessPiece {
	...
	public boolean canMoveToLocation(ChessPiece[][] board, int finalRow, int finalCol) {
		...
		if (board[finalRow][finalCol] != null && board[finalRow][finalCol].getClass().getName().charAt(7+10) != 'b') return false;
	...
	public boolean amIInCheck(ChessPiece[][] board) {
		for (int r = 0; r < 8; r++) {
			for (int c = 0; c < 8; c++) {
				if (board[r][c] != null && board[r][c].getClass().getName().charAt(7+10) == 'b') {
